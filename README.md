...
- **"How do we know this works on our actual CVs?"** Phase 1 of any customer engagement is eval on 50-100 of the customer's real CVs (anonymized or under DPA), producing a customer-specific F1 report before rollout decisions.
    ...
    **Held-out test set:** 11 synthetic PDF CVs with hand-validated ground truth.

    | | Macro F1 (raw) | Macro F1 (normalized) |
    |---|---|---|
    | Engineered pipeline | 0.80 | 0.86 |
    | Zero-shot baseline (gpt-4o, no schema) | 0.58 | 0.63 |
    | **Lift** | **+0.22** | **+0.23** |

    **Where the lift comes from:**
    - Schema enforcement eliminates key-naming drift in employment_history 
    and education (pipeline 0.98 vs baseline 0.00 on employer/institution 
    coverage)
    - Controlled vocabulary for set-valued fields (industries, functional_
    expertise, methods, tools) prevents free-text drift
    - Field-scoped prompt rules avoid cross-field constraint leakage (see 
    FAILURE_LOG 2026-05-08)

    **One failure mode persists in both:** `total_years_experience` at 0.00. 
    LLM date arithmetic over employment ranges is unreliable regardless of 
    prompt sophistication. v2 fix: deterministic post-processing.

    See [eval/results/EVAL_REPORT.md](eval/results/EVAL_REPORT.md) for full 
    per-CV breakdown and failure-mode analysis.

## What this demonstrates

**Resume-bullet** (one line for recruiters):  
Built CV-to-profile extraction pipeline with measured macro F1 = 0.80 on held-out test set, +0.22 lift over naive zero-shot baseline; failure-mode taxonomy and v2 roadmap documented in repo.

**Pre-sales framing** (for technical buyers):  
Schema-enforced LLM extraction beats zero-shot by 22 F1 points on the same 11 PDFs. The lift comes from five distinct mitigations: schema enforcement (keys + types), controlled vocabulary for set fields, field-scoped prompt rules, explicit output-language instruction, and an audit trail of ground-truth construction. Each mitigation maps to a specific failure mode in baseline output — see [FAILURE_LOG.md](FAILURE_LOG.md) and [eval/results/EVAL_REPORT.md](eval/results/EVAL_REPORT.md). One failure mode persists in both pipeline and baseline (date arithmetic for years-of-experience); v2 fix is deterministic post-processing.
