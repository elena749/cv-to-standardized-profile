...
🚧 **Status:** v1 shipped 12 May 2026. Eval complete (multi-run N=5, 2026-05-31). Loom demo in progress. Public deadline: 14 June 2026.
...
- [x] Run full pipeline against eval set — DONE 2026-05-12 (11 of 20 CVs, PDF only in v1)
- [x] Compute Field-Level F1 with baseline comparison — DONE 2026-05-12 (single-run), 2026-05-31 (multi-run N=5)
...
v1 was evaluated against 11 PDF CVs from the eval set (DOCX/TXT deferred to v2 — see Architecture Decisions).

**Multi-Run Reliability (N=5):**

| System | Raw F1 | Normalized F1 |
|---|---|---|
| Pipeline (schema-enforced) | **0.809 ± 0.006** | **0.861 ± 0.0004** |
| Baseline (zero-shot, no schema) | 0.567 ± 0.005 | 0.618 ± 0.005 |
| **Lift** | **+0.241** | **+0.243** |

Pipeline and baseline ranges do not overlap at n=5 — the lift is a resolvable improvement, not measurement noise. Both systems are highly stable across runs (pipeline normalized stdev = 0.0004 — near-deterministic thanks to schema-enforced structured outputs).

Full per-field breakdown, per-CV results, and failure-mode catalog: see [`eval/results/EVAL_REPORT.md`](eval/results/EVAL_REPORT.md).

Raw multi-run artifacts (per-run outputs + summary.json): `eval/multi_runs/20260531_165630/`

## What this demonstrates

**Resume-bullet** (one line for recruiters):  
Built CV-to-profile extraction pipeline with measured macro F1 = 0.81 on 11-CV eval set, +0.24 lift over naive zero-shot baseline; failure-mode taxonomy and v2 roadmap documented in repo.

**Pre-sales framing** (for technical buyers):  
Schema-enforced LLM extraction beats zero-shot by 24 F1 points on the same 11 PDFs. The lift comes from five distinct mitigations: schema enforcement (keys + types), controlled vocabulary for set fields, field-scoped prompt rules, explicit output-language instruction, and an audit trail of ground-truth construction. Each mitigation maps to a specific failure mode in baseline output — see [FAILURE_LOG.md](FAILURE_LOG.md) and [eval/results/EVAL_REPORT.md](eval/results/EVAL_REPORT.md). One failure mode persists in both pipeline and baseline (date arithmetic for years-of-experience); v2 fix is deterministic post-processing.
...
- 18 min/CV: midpoint between 10–15 (experienced recruiters) and 30 (juniors)
...
**Payback period:** Pipeline cost is ~$0.009 per CV at gpt-4o-2024-08-06 pricing. Monthly inference cost at 3,500 CVs: ~$32. Against €73,500/month of recruiter time saved, payback on infrastructure is effectively immediate; the meaningful investment is integration and change management, not API spend.

**Seasonal peak effect:** At 6,000–8,000 CVs/month in campus recruiting season, savings double — and the bottleneck becomes critical (processing slows, candidates wait too long, top profiles move to competitors).